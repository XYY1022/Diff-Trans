...
font_song = FontProperties(fname='C:\\Windows\\Fonts\\simsunb.ttf')
# 设置图例并且设置图例的字体及大小
...
def draw_pic(inv_y, hehe,PQJ, time_index, i):#oringin,iris1['安康入库2'],iris1['安康数据修正'],iris1['时间'],i
    ...
    plt.savefig("./pic_data_process/{}_1.png".format(i), dpi=300, bbox_inches='tight')
    #pyplot.show()
...
def smooth(a, WSZ):#oringin, 3
    a = np.squeeze(a) # 删除a中维度为1的条目
    # convolve运算逻辑见 https://www.cnblogs.com/redtank/p/15678714.html ，最终len(out0) = len(a)-WSZ + 1
    out0 = np.convolve(a, np.ones(WSZ, dtype=int), 'valid') / WSZ
    # np.arange(0,10,2)  0,2,4,6,8
    r = np.arange(1, WSZ - 1, 2)   # 1
    # np.cumsum(a,axis) 按指定的axis维度累加
    file_path = './data/wanan_0.xlsx'
    n_in_timestep = 7
    start = np.cumsum(a[:WSZ - 1])[::2] / r  # np.cumsum(a[])
...
def smooth_data():
    # 获得所有洪水编号并按照时间前后排序
    df = pd.read_excel('./allflood.xlsx')
    df1 = df.loc[:, ['FDNO']]
    ...
    df['QIN'] = qin_processed
    df['OT'] = df['QIN']
    df.to_excel("./allflood.xlsx",index=False)
